...
class RetrofitDataAgentImpl extends SmileShopDataAgent {
  ...
  Future<ProductVO> getProductDetails(
    ...
    return mApi
        .productDetail(
            'Bearer $token', acceptLanguage, endUserId, int.parse(productId))
        .asStream()
  ...
  Future<List<SubcategoryVO>> subCategoryByCategory(String token,
    ...
    return mApi
        .subCategoryByCategory(
            'Bearer $token', acceptLanguage, subCategoryRequest)
        .asStream()
        .map((response) => response.data ?? [])
        .first
        .catchError((error) {
  ...
  Future<List<ProductVO>> searchProductsWithDynamicParam(
      ...
      int? maxRange) {
    return mApi
        .searchWithDynamic('Bearer $token', acceptLanguage,
            int.parse(endUserId), pageNo, name, rating, minRange, maxRange)
        .asStream()
        .map((response) => response.data?.products ?? [])
        .first
        .catchError((error) {
  ...
  Future<SuccessNetworkResponse> postRefund(String token, String acceptLanguage,
    ...
    return mApi
        .postRefund('Bearer $token', acceptLanguage, orderNo,
            reasonId.toString(), userId.toString(), image!)
        .asStream()
        .map((response) {
          return response;
        })
  ...
  Future<SuccessNetworkResponse> changePassword(
      ...
      String passwordType) {
    return mApi
        .changePassword(token, acceptLanguage, endUserId, oldPassword,
            newPassword, confirmPassword, passwordType)
        .asStream()
        .map((response) => response)
        .first
        .catchError((error) {
